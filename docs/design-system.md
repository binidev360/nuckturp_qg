...
>
> **Erratas pós-inventário (2026-05-29):** valores reais validados em [inventario/ui-componentes.md](inventario/ui-componentes.md) e portados em `app/globals.css` (Tailwind v4). `FloatingDice` não existe no código atual (D20 = `DiceIcons.tsx` + PNG; criar na Fase 5); vars nomeadas (`--cyber-lime` etc.) herdam o light no `.dark`; motion atual é CSS-first (keyframes + IntersectionObserver), não framer-motion.
